...
def set_app_background(image_path: str):
    """Embed a local image as the app background (works locally & on Streamlit Cloud)."""
    try:
        path = Path(image_path)
        ext = (path.suffix or "").lower().lstrip(".") or "jpg"
        ...
        st.markdown(
            ...
            [data-testid="stAppViewContainer"] {{
                ...
                background: 
                    linear-gradient(135deg, rgba(245,247,250,0.85) 0%, rgba(195,207,226,0.85) 100%),
                    url("{data_url}");
        ...
        st.markdown("<h1 style='text-align: center;'>PeNNy PaTh</h1>", unsafe_allow_html=True)
...
set_app_background("assets/coin-jar.jpg")
...
def page_nlu():
    ...
    st.caption("Enter your message below to analyze sentiment, keywords, and entities:")
    ...
    if run:
        ...
        with st.spinner("Analyzing..."):
            result = nlu_analyze_text(user_text, DEFAULT_MODEL)
...
def page_qa():
    ...
    st.caption("Ask your personal finance question below:")

    question = st.text_area("❓ Your Question", height=120)
    persona = st.selectbox("👤 Your Persona", options=["General", "Professional", "Student", "Retired"], index=0)
    ...
    if run:
        ...
        with st.spinner("Thinking..."):
            # Optional NLU analysis on question
            nlu = nlu_analyze_text(question, DEFAULT_MODEL)
            answer = hf_generate(build_qa_prompt(question, persona.lower()), model_id=model_id, max_new_tokens=450, temperature=0.2)
...
# ---- Budget Summary (User-friendly inputs) ----
def page_budget():
    ...
    st.markdown("Enter your annual income and expenses below:")

    # Input fields
    income = st.number_input("💰 Annual Income", value=sample["income"], min_value=0, step=1000)
    rent = st.number_input("🏠 Rent", value=sample["rent"], min_value=0, step=500)
    groceries = st.number_input("🛒 Groceries", value=sample["groceries"], min_value=0, step=500)
    transport = st.number_input("🚗 Transport", value=sample["transport"], min_value=0, step=500)
    dining_out = st.number_input("🍽️ Dining Out", value=sample["dining_out"], min_value=0, step=500)
    utilities = st.number_input("💡 Utilities", value=sample["utilities"], min_value=0, step=500)
    subscriptions = st.number_input("📺 Subscriptions", value=sample["subscriptions"], min_value=0, step=500)

    # Optional: other expenses
    st.markdown("You can add other expenses (optional):")
    other_exp = st.text_area("Other Expenses (comma-separated, e.g., insurance:5000,healthcare:10000)", height=60)

    run = st.button("💹 Compute Budget")
    back = st.button("⬅️ Back")
    ...
    if run:
        # Combine all inputs into a dict
        data = {
            "income": income,
            "rent": rent,
            "groceries": groceries,
        ...
        if other_exp.strip():
            try:
                for item in other_exp.split(","):
                    if ":" in item:
                        key, val = item.split(":", 1)
                        data[key.strip()] = float(val.strip())
            except Exception:
                st.error("Please format 'Other Expenses' correctly as key:value pairs separated by commas.")
                return
